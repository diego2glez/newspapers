...
while $nodescargado; do
	sleep 2
	wget -q --load-cookies cookitcentro --max-redirect 0 "http://digital.ilcentro.it/ilcentro/books/chieti/"${yy}"/"${fecha}"chieti/" 2>/dev/null
	if [ $? -eq 0 ]; then
		while $nodescargado; do
		wget -nv --load-cookies cookitcentro "${url}/ilcentro/books/"${fecha}"chieti/singole/"${pag}".pdf&download=true" -O ${carpetatmp}/Pag${pag}.pdf		
		if [ $? -eq 0 ]; then
