...
while $nodescargado; do
	sleep 2
	wget -q --load-cookies cookitcentro --max-redirect 0 "http://digital.ilcentro.it/ilcentro/books/pescara/"${yy}"/"${fecha}"pescara/" 2>/dev/null
	if [ $? -eq 0 ]; then
		while $nodescargado; do
		wget -nv --load-cookies cookitcentro "${url}/ilcentro/books/"${fecha}"pescara/singole/"${pag}".pdf&download=true" -O ${carpetatmp}/Pag${pag}.pdf		
		if [ $? -eq 0 ]; then
